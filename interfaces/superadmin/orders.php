...
// Determine user type and set owner filter (use numeric owner_id)
$orders = [];
$owner_filter = null;

if ($_SESSION['role'] === 'superadmin') {
    // Superadmin should only see orders they own
    $owner_filter = intval($_SESSION['super_id']);
} elseif ($_SESSION['role'] === 'admin') {
    // Admin should only see orders for their store
    $owner_filter = intval($_SESSION['admin_id']);
}

// Fetch orders for this owner (both admin and superadmin)
