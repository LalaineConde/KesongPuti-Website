...
// Ensure super_id is available
if (!isset($_SESSION['super_id'])) {
    $_SESSION['super_id'] = 0; // fallback
}
$super_id = intval($_SESSION['super_id']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_category') {
    ...
    $stmt = $connection->prepare("SELECT category_id FROM categories WHERE LOWER(category_name) = LOWER(?) LIMIT 1");
    ...
    $stmt = $connection->prepare("INSERT INTO categories (category_name, owner_id, owner_type, created_at) VALUES (?, ?, 'superadmin', NOW())");
    $stmt->bind_param('si', $category_name, $super_id);
...
if (isset($_GET['action']) && $_GET['action'] === 'fetch_categories') {
    header('Content-Type: application/json');
    $cats = [];
    $res = $connection->query("SELECT category_id, category_name FROM categories WHERE owner_type='superadmin' OR owner_type='admin' ORDER BY category_name ASC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_product') {
    ...
    $stmt = $connection->prepare(
        "INSERT INTO products (product_name, description, price, stock_qty, product_image, category_id, owner_id, owner_type, date_added, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)"
    );
    $owner_type = 'superadmin';
    $stmt->bind_param('ssdisiiss', $name, $desc, $price, $stock, $main_img_path, $cat_id, $super_id, $owner_type, $status);
    ...
    // Variants (with individual images)
    if (!empty($_POST['variant_size']) && is_array($_POST['variant_size'])) {
        ...
        foreach ($_POST['variant_size'] as $i => $size) {
            ...
            $stmt3 = $connection->prepare("
                INSERT INTO product_variations (product_id, size, price, stock_qty, variant_image)
                VALUES (?, ?, ?, ?, ?)
            ");
...
// Products for grid (only products owned by this superadmin)
...
$categories = $connection->query("SELECT * FROM categories WHERE owner_type='superadmin' OR owner_type='admin' ORDER BY category_name ASC");
$storeResult = $connection->query("SELECT store_name FROM store ORDER BY store_name ASC");
?>
