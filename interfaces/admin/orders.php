...
$orders = [];
$orders_query = "
    SELECT o.o_id, o.reference_number, o.order_date, o.order_status,
              ...
              <tr data-status="<?php echo htmlspecialchars($order['order_status']); ?>" data-type="<?php echo htmlspecialchars($order['order_type']); ?>">
                ...
                <td>
                  <select class="order-status" 
                          ...
                          data-customer-name="<?= $order['fullname']; ?>"
...
<script>
  const loggedInAdminEmail = "<?php echo htmlspecialchars($admin_email); ?>";
...
function displayOrderDetails(order) {
    ...
    const loggedInAdminEmail = "<?php echo $_SESSION['email']; ?>"; // admin's email from session
  ...
  dropdowns.forEach(select => {
    select.addEventListener("change", async function () {
      ...
      // Update database
      try {
        const response = await fetch("orders-email-admin.php", {
          method: "POST",
